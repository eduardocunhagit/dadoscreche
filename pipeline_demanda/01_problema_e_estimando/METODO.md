# Frente 2 — escolha da unidade e concorrência

## 1. Pergunta e estimando

A Frente 2 estima como a demanda **já inscrita** se distribui entre alternativas de creche. Seu estimando principal é a probabilidade de uma alternativa ser a primeira opção registrada, condicional à inscrição, ao grupamento, ao ano, ao território de origem e ao conjunto de alternativas elegíveis.

Esse objeto não é demanda potencial, matrícula nem alocação:

- **entrada na rede:** quantidade de crianças que se inscrevem; estimada pela Frente 1;
- **preferência submetida:** ordem de unidades e turnos registrada pela família; objeto da Frente 2;
- **demanda prevista por alternativa:** entrada prevista multiplicada pela participação prevista pela Frente 2;
...
A Frente 2 não altera a prioridade socioeconômica e não produz ranking de crianças. Seu produto é uma previsão explicável para planejamento de oferta.
...
Também serão reportados MAE, WAPE e erro de participação por unidade. A avaliação condicional isola a distribuição entre alternativas, sem atribuir à Frente 2 o erro no total de inscritos.
...
## 9. Integração com a Frente 1

### 9.1 Definições adicionais

| Símbolo | Definição |
| --- | --- |
| `\widehat A_{mgt}` | Número de inscrições previsto pela Frente 1 para território `m`, grupamento `g` e ano `t`. |
| `\widehat s_{amgt}` | Participação prevista pela Frente 2 para alternativa `a`, território `m`, grupamento `g` e ano `t`. |
...
A comparação da demanda integrada com a histórica só ocorre quando a previsão OOS da Frente 1 estiver disponível. Antes disso, `\widehat D_{agt}^{cond}` é o produto correto para avaliar isoladamente a Frente 2.
...
11. **Integração:** aplicar as participações congeladas à saída da Frente 1.
...
- A Frente 2 não determina classificação, alocação ou convocação.
