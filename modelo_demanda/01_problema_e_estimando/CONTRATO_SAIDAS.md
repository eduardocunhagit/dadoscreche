# Contrato de saídas da modelo de escolha
...
Este arquivo é produzido pela etapa de integração, não pela estimação isolada da modelo de escolha.

**Grão:** `run_id × model_id × target_year × group × alternative_id × draw_id`.

| Campo | Definição |
| --- | --- |
| `draw_id` | Identificador do sorteio de incerteza recebido da modelo de demanda potencial e, quando implementado, da modelo de escolha. |
| `predicted_entries` | Total territorial previsto pela modelo de demanda potencial antes da distribuição. |
| `choice_share` | Participação prevista pela modelo de escolha. |
...
3. demanda integrada quando a modelo de demanda potencial estiver disponível;
